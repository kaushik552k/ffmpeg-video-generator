/**
 * demo_fade_new.ts — NEW Native FFmpeg fade filter
 * ─────────────────────────────────────────────────
 * Uses FFmpeg's built-in `fade` filter for fade/zoom animations.
 * The native `fade` filter uses SIMD CPU instructions (SSE/AVX) to
 * process 16-32 pixels SIMULTANEOUSLY in a single instruction —
 * vs geq which processes 1 pixel at a time.
 *
 * Same composition. Same animations. Just a smarter filter.
 *
 * Run: npx tsx src/test/demo_fade_new.ts
 */
...
// ── Shared composition (identical to demo_geq_old.ts) ────────────────────────
...
async function main() {
    console.log('\n' + '═'.repeat(60));
    console.log('  🚀 DEMO — NEW Approach  (FFmpeg native fade — SIMD)');
    console.log('═'.repeat(60));
    console.log('  Filter   : fade=type=in:start_time=X:duration=Y:alpha=1');
    console.log('  Cost     : SIMD vector ops — 16-32 pixels per instruction');
    console.log('  Layers   : 5 (video + text + list + table + image)');
    console.log('  Animations: fadeIn/Out · slideInLeft/Right/Bottom · zoomIn/Out');
    console.log('─'.repeat(60) + '\n');
    console.log('  ⚡ Video served from local cache (no download)\n');
    ...
    try {
        ...
        console.log(`  ⏱️  Time   : ${elapsed}s   ← THIS IS THE FAST APPROACH`);
        console.log('═'.repeat(60) + '\n');
    } catch (err) {
        console.error('\n❌ Error:', err);
        process.exit(1);
